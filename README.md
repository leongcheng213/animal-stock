...
3. Bell → reveal: stocks flip and **only the newest order's animal is judged**. Oversold ⇒ last orderer takes the token; otherwise the ringer does. Tokens go 1,2,3… first to **7+ loses**, fewest wins.
4. Hippos 🦛: drawn = tap a face-up order **on the board** to swap its picked/unpicked halves (the Hippo parks beside that row as a marker). No orders ⇒ naps, no effect. **No ringing on the turn right after a Hippo** (bell locks, bots obey too). In stock at reveal: **Bo** cancels all 3-counts, **Pip** cancels all Zebra, **Dozy** naps. Swapping places no order, so it can never make *you* the blamed player that round.
...
- **Bell-check rule:** the bell judges **only the newest order's animal** (simpler, faster than the spec's §8 every-animal check). The reveal highlights that judged column.
